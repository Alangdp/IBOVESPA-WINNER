...
export class BazinMethod {
    ...
    constructor(stock) {
        this.stock = stock;
        const { dividendYield, actualPrice, lastDividendsYieldYear, lastDividendsValueYear, activeValue, } = this.stock;
    ...
    crescentDividend() {
        ...
        for (let i = 0; i < lastDividendsValue.length; i++) {
            if (lastDividendsValue[i + 1] === undefined)
                break;
    ...
    makePoints() {
        ...
        const { grossDebt, patrimony, actualDividendYield, payout, actualPrice } = this.stock;
        const conditions = {};
        const maxPrice = (this.dividendYieldAverage * this.stock.actualPrice) / 0.06;
