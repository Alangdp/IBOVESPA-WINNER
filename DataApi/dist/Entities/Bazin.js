import { BazinProtocol, } from '../interfaces/BazinProtocol.type.js';
// TODO - REFAZER TUDO
export class Bazin extends BazinProtocol {
    constructor(stock) {
        super();
        const { dividendYield, actualPrice, lastDividendsYieldYear, lastDividendsValueYear, activeValue, } = stock;
    ...
    addPoints(validation, toAdd, toRemove) {
        if (validation) {
            this.points += toAdd;
            return validation;
        }
    ...
    makePoints(stock) {
        ...
        const { grossDebt, patrimony, actualDividendYield, payout, actualPrice } = stock;
        const conditions = {};
        ...
        conditions['Dividend Yield Atual > 0.06 (6%)'] = this.addPoints(actualDividendYield >= 0.06, 1, 2);
        conditions['Dívida Bruta/Patrimônio < 0.5 (50%)'] = this.addPoints(grossDebt / patrimony <= 0.5, 1, 2);
        conditions['Pagamento constante de dividendos nos últimos 5 anos'] =
            this.addPoints(this.constistentDividend(), 1, 1);
        conditions['Dividendos crescentes nos últimos 5 anos'] = this.addPoints(this.crescentDividend(), 1, 1);
        conditions['0 < Payout < 1'] = this.addPoints(payout > 0 && payout < 1, 1, 1);
        conditions['Preço Atual < Preço Máximo'] = this.addPoints(actualPrice < maxPrice, 1, 1);
        const points = {};
