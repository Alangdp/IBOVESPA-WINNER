...
export class GranhamMethod {
    ...
    makePoints() {
        ...
        conditions['Valores de "Net Liquid" foram positivos em todos os últimos 10 anos'] = netLiquidOn10Years.every((netLiquid) => netLiquid.value > 0);
        ...
        conditions['Fórmula de Benjamin Graham para Valor Intrínseco (Preço Justo)'] = Math.sqrt(22.5 * vpaAverage * lpaAverage) > 1.5 * stock.actualPrice;
    ...
    crescentNetLiquid(netLiquidOn10Years) {
        let crescent = true;
        for (let i = 0; i < netLiquidOn10Years.length; i++) {
            if (netLiquidOn10Years[i + 1] === undefined)
                break;
    ...
    crescentLpa() {
        ...
        const lpaFinal = (lpa[lpa.length - 3] + lpa[lpa.length - 2] + lpa[lpa.length - 1]) / 3;
    ...
    calculateYearGrowth(numberYears) {
        try {
            ...
            const actualNetLiquid = netLiquid.find((netLiquid) => netLiquid.year === actualYear);
            const lastNetLiquid = netLiquid.find((netLiquid) => netLiquid.year === lastYear);
            if (!actualNetLiquid || !lastNetLiquid)
                throw new Error('Invalid NetLiquid');
            const growth = (actualNetLiquid.value - lastNetLiquid.value) / lastNetLiquid.value;
            if (growth > 0.05)
                return true;
            return false;
        }
