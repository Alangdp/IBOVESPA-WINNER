...
const getHistory: RequestHandler = async (req, res, next) => {
  try {
    ...
    const transactionsData = await transactionService.findAllByUserToken(token);
    if (!transactionsData) throw new Error('Invalid Token');
    const history: HistoryProps =
      await History.instanceHistory(transactionsData);
    ...
    const filteredHistoryData = Object.fromEntries(
      Object.entries(historyData).map(([date, data]) => [
        date,
        {
          chart: historyData[date].chart,
          dividends: historyData[date].dividends,
        },
      ])
    );
    return response(res, {
      status: 200,
      data: { chart, historyData: filteredHistoryData, transactions },
    });
  } catch (error: any) {
    console.log(error);
